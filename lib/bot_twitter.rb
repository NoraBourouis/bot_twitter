...
#client.update('Test de tweet avec API #developpeuseEnHerbe')
return client
end

#comptes = ["@Nora23041994","@StengerMarylis","@Maxime_Smo", "@HamdiNassri"] #creer un array
#i = comptes.sample(2) #recuperer deux items du tableau
 #   i.each do |x| #pour chaque item récupéré
  #    login_twitter.update("#{x} It's a tweet!") #mettre le pseudo + tweet, tout en récuperant la connexion
   # end

def faire_tweet(login_twitter)
...
i = journalistes.sample(5)
    i.each do |x|
      login_twitter.update("#bonjour_monde, #{x} J'aime beaucoup ce que vous faites. :) ")
    end
  end

  #def perform
   # login_twitter  
    #faire_tweet(login_twitter)
    #like
    #end

#perform 

def like(login_twitter) 
posts = login_twitter.search("#bonjour_monde", result_type: "recent").take(25).collect #on prend les 25 derniers posts avec le # bonjour_monde
  posts.each do |x| #pour chacun de ces tweets
    login_twitter.fav x # on les fav
  end
end 



def follow(login_twitter)
  login_twitter.search("#bonjour_monde").take(20).each do |x| #on prend les 20 messages contenant #bonjour_monde et pour chaque message
  login_twitter.follow(x.user) #on follow le user du message
  end
end
...
#def follow(login_twitter)
 # n = 0
  #until n = 20 do
   # login_twitter.search("#bonjour_monde").take(1).collect do |x|
    #login_twitter.follow(x.user) 
    #end
    #n = +1
  #end
#end
...
def perform
  ...
  perform
#definir le streaming car ce n'est pas la meme fonction : avant c'est REST(Twitter::REST::Client) et maintenant c'est Streaming(Twitter::Streaming::Client)
...
client = Twitter::Streaming::Client.new do |config| 
  ...
  return client #c'est bon c'est configuré

end

# on cherche à liker et follower les personnes ayant tweeter #bonjour_monde en continu
def streaming
topics = "#bonjour_monde" #on defini une variable qu'on va recuperer
stream.filter(track: topics) do |object| #pour cela on traque les objets(message,tweet, et le stream) avec le contenu souhaité
 if object.is_a?(Twitter::Tweet) #si c'est un tweet = c'est bon, on peut le follow et liker
  stream.fav object
  stream.follow(object.user)
  end
end

streaming #appel de def
